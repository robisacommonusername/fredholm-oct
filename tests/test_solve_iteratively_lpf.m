%Boilerplate
function ret = test_solve_iteratively_lpf(varargin)
...
function [status, msg] = test_lowpass_sine_solution()
	...
	y = Kdag\rhs;
	wc = 4*pi;
	xp = solve_iteratively_lpf(K, Kdag, y, t, weights, eps, wc);
...
function [status, msg] = test_projected_solution()
	...
	y = Kdag\rhs;
	wc = 10*pi;
	x_l = solve_iteratively_lpf(K, Kdag, y, tbar, weights, eps, wc);
